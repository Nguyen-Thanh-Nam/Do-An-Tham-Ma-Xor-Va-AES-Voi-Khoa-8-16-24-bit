...
def key_expansion(key):
    ...
    for i in range(4, 44):
        ...
        if i % 4 == 0:
            ...
            t[0] ^= Rcon[i//4]
        word = [x^y for x,y in zip(ks[(i-4)*4:(i-3)*4], t)]
        ks += word
    return [ks[i*4:(i+1)*4] for i in range(44)]

def encrypt_block(p, w):
    s = list(p)
    s = add_round_key(s, sum(w[0:4], []))
    for r in range(1, 10):
        s = sub_bytes(s)
        s = shift_rows(s)
        s = mix_columns(s)
...
def fix_key(k):
    k = k.encode()
    return k + b'\x00' * (16 - len(k)) if len(k) < 16 else k[:16]
